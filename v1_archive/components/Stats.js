...
export default function Stats({route, navigation}) {
  ...
  const [oneMonthCalendarAndChart, setOneMonthCalendarAndChart] =
    useState(true);
  const [threeMonthCalendarAndChart, setThreeMonthCalendarAndChart] =
    useState(false);
  const [sixMonthCalendarAndChart, setSixMonthCalendarAndChart] =
    useState(false);
  const [twelveMonthCalendarAndChart, setTwelveMonthCalendarAndChart] =
    useState(false);
    ...
    for (let i = 0; i < howManyMonths; i++) {
      ...
      if (allMonthsData[i] && allMonthsData[i].monthData) {
        ...
        currentMonthCalendar.map((weekObject, weekIndex) => {
          for (let j = 0; j < weekObject.week.length; j++) {
            // Fixed the variable name to avoid conflicts
            if (
  ...
  const [toggleTwelveMonthCalendar, setToggleTwelveMonthCalendar] =
    useState(false);
  const [toggleAllTimeCalendar, setToggleAllTimeCalendar] = useState(false);
  // for when loading the calendar popup
  const [toggleLoadingMonthsCalendars, setToggleLoadingMonthsCalendars] =
    useState(false);
                ...
                <View
                  ...
                  <View
                    style={{
                      ...
                      flexDirection: 'row',
                      justifyContent:
                        index === userData.currentMonthCalendar.length - 1
                          ? 'flex-start'
                          : 'space-between',
                      alignItems: 'center',
                      alignSelf: 'stretch',
                    ...
                    {month.monthData.map((weekArray, index) => (
                      <View
                        ...
                        style={{
                          ...
                          flexDirection: 'row',
                          justifyContent:
                            index === month.monthData.length - 1
                              ? 'flex-start'
                              : 'space-between',
                          alignItems: 'center',
                          alignSelf: 'stretch',
